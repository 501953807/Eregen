...
住院链患者身份存储于统一 `persons` 表，`id_card` 为必填字段（唯一标识）。
...
1. 扫描/输入身份证号
   ↓
2. 系统查询 persons 表
   ├─ 不存在 → 新人员，需完整填写（姓名、性别、出生日期、电话、紧急联系人）
   └─ 已存在 → 自动填充基础信息，仅需录入住院业务字段
   ↓
3. 生成住院档案（person_profiles, business_chain='hospital'）
4. 分配腕带设备（medical_wristband）
5. 写入入院记录（hospital_admissions）
```

**跨链复用示例：**
```
张三个人（身份证号：110101196001011234）
├─ 住院链：心内科 12床，2026-08-01 入院
├─ 出院后 → 住院链 status='discharged'
└─ 同一人参加社区福利 → 社区链自动填充姓名/身份证/电话，仅录入福利标签
```
...
| 设备类型 | 用途 | 连接方式 |
|---------|------|---------|
| medical_wristband（医用腕带） | 患者身份标识、电子围栏 | BLE/NFC + WiFi/Cat1 |
| nurse_terminal（护士终端） | NFC扫描核验、查房记录 | 独立PDA设备 |

住院链不使用手环（bracelet）和药盒（pillbox），这些属于自营链。

### 11.3 权限隔离

- **nurse** 角色：仅能访问住院链数据，看不到自营链和社区链
- **hospital_doc** 角色：可查看和编辑住院链数据
- **super_admin** 和 **regulator**：可跨链查看

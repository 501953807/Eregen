      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const Expanded(
              child: Text('健康数据', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
            ),
      ...
      child: Padding(
        ...
        child: Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [const Color(0xFF4A90D9), const Color(0xFF357ABD)]),
            borderRadius: BorderRadius.circular(16),
          ),
          padding: const EdgeInsets.all(20),
          child: Column(
            children: [
              const Text('综合健康风险评估', style: TextStyle(fontSize: 13, color: Colors.white)),
              ...
              Text(
                _riskSummary(),
                style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.85), height: 1.5),
              ),
            ],
          ),
        ),
  ...
  Widget _buildSpO2() {
    return SliverToBoxAdapter(
      child: _metricCard(
        ...
        unit: '%',
        badge: _latestSpo2 != null
            ? BadgeChip(
                label: _latestSpo2! >= 95 ? '正常' : '偏低',
                color: _latestSpo2! >= 95 ? AppTheme.statusNormal : AppTheme.statusWarning,
              )
            : null,
  ...
  Widget _buildStepsSummary() {
    return SliverToBoxAdapter(
      child: Padding(
        ...
        child: Row(
          children: [
            _stepStat('\u{1F6B6}', '${(_latestSteps ?? 0).toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (m) => '${m[1]},')} 步', '今日步数'),
            const SizedBox(width: 12),
            _stepStat('\u{1F525}', '186', '千卡消耗'),
            const SizedBox(width: 12),
            _stepStat('\u{23F1}', '42', '活动分钟'),
          ],
        ),
      ),
    );

...
/// Blood sugar (glucose) detail page — line chart with normal-range banding,
/// daily measurement records list, and current reading summary.
class BloodSugarPage extends StatefulWidget {
  final String elderlyId;
  final String elderlyName;
  const BloodSugarPage({super.key, required this.elderlyId, required this.elderlyName});
...
class _BloodSugarPageState extends State<BloodSugarPage> {
  ...
  // Normal range: fasting 3.9–6.1 mmol/L
  static const double _normalLow = 3.9;
  static const double _normalHigh = 6.1;
  static const double _chartMinY = 2.0;
  static const double _chartMaxY = 10.0;
  static const String _unit = 'mmol/L';

  // Latest reading
  double? _latestValue;
  DateTime? _latestTime;

  // Chart data points [x = epoch seconds, y = glucose]
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final resp = await ApiClient.instance.get(
        '/api/v1/chronic/${widget.elderlyId}/glucose',
        query: {'days': 30},
      );
      ...
      setState(() {
        ...
        for (int i = 0; i < items.length; i++) {
          ...
          final val = double.tryParse((r['value'] ?? r['glucose'] ?? '0').toString()) ?? 0.0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('${widget.elderlyName} · 血糖详情'),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Center(child: Text('\u{1F4A1}', style: TextStyle(fontSize: 26))),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text('当前血糖', style: TextStyle(fontSize: 13, color: const Color(0xFF6B7280))),
                const SizedBox(height: 4),
                RichText(
                  text: TextSpan(
                    style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: Color(0xFF1F2937)),
                    children: [
                      TextSpan(text: _latestValue!.toStringAsFixed(1)),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: const [
              Text('空腹血糖趋势（近30天）', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF1F2937))),
              Text('单位: mmol/L', style: TextStyle(fontSize: 12, color: Color(0xFF9CA3AF))),
            ],
          ),
...
class _RecordTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = record.value < 3.9
        ? AppTheme.statusInfo
        : (record.value > 6.1 ? AppTheme.statusDanger : AppTheme.statusNormal);
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Center(child: Text('\u{1F4A1}', style: const TextStyle(fontSize: 18))),
          ),
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        record.value < 3.9 ? '偏低' : (record.value > 6.1 ? '偏高' : '正常'),
          ...
          Column(
            ...
            children: [
              ...
              const Text('mmol/L', style: TextStyle(fontSize: 9, color: Color(0xFFD1D5DB))),
            ],
          ),
        ],
      ),

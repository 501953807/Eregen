...
class _ElderlyDetailPageState extends State<ElderlyDetailPage> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      futures.add(ApiClient.instance.get('/admin/elderly').then((resp) {
        if (resp.data != null && (resp.data as Map).containsKey('data')) {
          final list = resp.data['data'] as List;
          if (list.isNotEmpty) {
            setState(() => _healthStats = list[0]);
          }
        }
      }).catchError((_) {}));
              ...
              slivers: [
                ...
                SliverToBoxAdapter(
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        const CircleAvatar(radius: 32, backgroundColor: AppTheme.primary, child: Icon(Icons.person, size: 32, color: Colors.white)),
                        const SizedBox(width: 16),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('李秀英 奶奶', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                              const SizedBox(height: 4),
                              Row(
                                children: [
                                  _statusChip('中风险', AppTheme.statusWarning),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          runSpacing: 10,
                          children: [
                            _miniStatCard('心率', '72 bpm', AppTheme.primary, Icons.favorite),
                            _miniStatCard('血氧', '97%', AppTheme.statusNormal, Icons.air),
                            _miniStatCard('步数', '3,456', AppTheme.statusWarning, Icons.directions_walk),
                            _miniStatCard('电量', '85%', AppTheme.statusInfo, Icons.battery_full),
                          ],
                        ),
                      ],
                    ),
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      // TODO: API call to create elderly profile

...
def main():
    ...
    # Step 3: Create persons (they already exist from previous runs)
    log("\n[3] Creating persons...")
    person_ids = {}
    for elder in ELDERS:
        existing = None
    ...
    # Step 5: Create medication rules
    ...
    # Step 7: Create hospital patients
    log("\n[7] Creating hospital patients...")
    total_patients = 0
    for elder in ELDERS:
    ...
    # Step 8: Create community elders
    log("\n[8] Creating community elders...")
    total_community = 0
    for elder in ELDERS:
    ...
    # Final counts
    data = api_request(token, "GET", "/admin/persons")
    log(f"  Persons: {len(data.get('data', [])) if data else 0}")
    data = api_request(token, "GET", "/admin/medical/patients")
    log(f"  Hospital patients: {len(data.get('data', [])) if data else 0}")
    data = api_request(token, "GET", "/admin/community-wb/elders")
    log(f"  Community elders: {len(data.get('data', [])) if data else 0}")
    data = api_request(token, "GET", "/admin/alerts")
    log(f"  Alerts: {len(data.get('data', [])) if data else 0}")

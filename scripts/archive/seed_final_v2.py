...
def main():
    log("=" * 70)
    log("Eregen Data Seed (Direct DB - Fixed)")
    log("=" * 70)
    ...
    # 1. Health records (bulk insert for all self-chain persons)
    ...
    log("\n[3] Creating hospital admissions...")
    total_admissions = 0
    for elder in ELDERS:
        if "dept" not in elder:
            continue
        # Find or create medical patient
        patient_id = None
        cur.execute("SELECT id FROM medical_wristband_patients WHERE admission_no LIKE ?", (f"H%",))
        # Check if patient exists by name
        cur.execute("SELECT id FROM medical_wristband_patients WHERE name=?", (elder["name"],))
        row = cur.fetchone()
        if row:
            patient_id = row[0]
            log(f"  ✓ Patient exists: {elder['name']}")
        else:
            ...
            try:
                ...
                log(f"  ✗ Failed to create patient: {e}")
                continue

        # Create admission
        try:
            cur.execute("""
                INSERT INTO hospital_admissions (id, patient_id, admission_no, bed_no, department, diagnosis, admitted_at, expected_discharge_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                str(uuid.uuid4()), patient_id,
                f"H{random.randint(10000, 99999)}",
                f"{random.randint(1, 30)}床", elder.get("dept", "内科"),
                "待诊断",
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                (datetime.now() + timedelta(days=random.randint(3, 14))).strftime("%Y-%m-%d %H:%M:%S")
            ))
            total_admissions += 1
    ...
    conn.commit()
    log(f"  Total admissions: {total_admissions}")

    # 4. Medical daily entries
    log("\n[4] Creating medical daily entries...")
    total_daily = 0
    for elder in ELDERS:
        if "dept" not in elder:
            continue
        ...
        for _ in range(random.randint(3, 5)):
            try:
                ...
                pass
    conn.commit()
    log(f"  Total daily entries: {total_daily}")

    # 5. Medical verifications
    log("\n[5] Creating medical verifications...")
    ...
    for elder in ELDERS:
        ...
        for _ in range(random.randint(5, 10)):
            try:
                ...
                pass
    conn.commit()
    log(f"  Total verifications: {total_verifications}")

    # 6. Medical expenses
    log("\n[6] Creating medical expenses...")
    total_expenses = 0
    for elder in ELDERS:
        if "dept" not in elder:
            continue
        ...
        for _ in range(random.randint(5, 15)):
            try:
                ...
                pass
    conn.commit()
    log(f"  Total expenses: {total_expenses}")

    # 7. Community sign-ins
    log("\n[7] Creating community sign-ins...")
    total_signins = 0
    for elder in ELDERS:
        if "welfare" not in elder:
            continue
        cur.execute("SELECT id FROM community_elders WHERE id_card=?", (elder["id_card"],))
        row = cur.fetchone()
        if not row:
            log(f"  ✗ Community elder not found: {elder['name']}")
            continue
        elder_id = row[0]
        for day in range(14):
            for period in ["morning", "afternoon"]:
                try:
                    ...
                    """, (
                        ...
                        "INST-001",
                        (datetime.now() - timedelta(days=day, hours=random.randint(0, 12))).strftime("%Y-%m-%d %H:%M:%S"),
                        period, 1, 1
                    ))
                    total_signins += 1
                except:
                    pass
        log(f"  ✓ {elder['name']}: 28 sign-ins")
    conn.commit()

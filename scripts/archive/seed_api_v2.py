...
def api_request(token, method, path, data=None, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 200 or resp.status_code == 201 or resp.status_code == 204:
                ...
                log(f"  ⚠ Rate limited on {method} {path}, waiting {wait_time}s...")
                time.sleep(wait_time)
                continue
            elif resp.status_code == 400:
                # Log the error but don't retry on 400 (bad request)
                log(f"  ✗ {method} {path} -> 400: {resp.text[:200]}")
                return None
            else:
                log(f"  ✗ {method} {path} -> {resp.status_code}: {resp.text[:200]}")
                return None
        except Exception as e:
            log(f"  ✗ API error on {method} {path}: {e}")
...
def main():
    log("=" * 70)
    log("Eregen Full Chain Data Simulation (API-Based) - Fixed Version")
    log("=" * 70)
    ...
    # Step 5: Create devices and bindings
    log("\n[5] Creating devices and bindings...")
    device_ids = {}
    ...
    # Step 6: Create health records (batch mode to avoid rate limiting)
    log("\n[6] Creating health records...")
    total_health = 0
    ...
    for elder in ELDERS:
        ...
        for i in range(count):
            ...
            # Rate limit protection - sleep every 5 records
            if i % 5 == 4:
                time.sleep(0.3)

        total_health += success_count
        log(f"  ✓ {elder['name']}: {success_count}/{count} health records")

    log(f"  Total health records created: {total_health}")

    # Step 7: Create medication rules
    log("\n[7] Creating medication rules...")
    meds = [
    ...
    # Step 8: Create hospital patients and data
    log("\n[8] Creating hospital data...")
    ...
    # Step 9: Create community elders and sign-ins
    log("\n[9] Creating community data...")
    total_signins = 0
    ...
    for elder in ELDERS:
        ...
        # Create sign-in records (limit to avoid rate limiting)
        for day in range(7):  # Reduce from 14 to 7 to avoid rate limiting
            for period in ["morning", "afternoon"]:
                ...
                resp = api_request(token, "POST", "/admin/community-wb/signin/trigger", payload, max_retries=1)
                if resp and resp.get("code") == "OK":
                    total_signins += 1
                time.sleep(0.05)
    ...
    for elder in ELDERS:
        ...
        alert_count = random.randint(2, 4)
    ...
    log("=" * 70)
    log(f"Summary:")
    log(f"  - Health records: {total_health}")
    log(f"  - Medication rules: {total_meds}")
    log(f"  - Hospital verifications: {total_verifications}")
    log(f"  - Hospital daily entries: {total_daily}")
    log(f"  - Hospital expenses: {total_expenses}")
    log(f"  - Community sign-ins: {total_signins}")

...
def main():
    log("=" * 70)
    log("Eregen Final Data Seed (Correct Schema)")
    log("=" * 70)
    ...
    # 1. Health records (bulk insert with correct schema)
            ...
            try:
                cur.execute("""
                    INSERT INTO health_records (id, person_id, business_chain, record_type, source,
                                               ...
                                               steps, sleep_hours, recorded_at, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
            ...
            try:
                cur.execute("""
                    INSERT INTO medication_rules (id, person_id, business_chain, drug_name, generic_name,
                                                  ...
                                                  drug_category, active, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    str(uuid.uuid4()), pid, "self", med[0], med[1], med[2], med[3], med[4], med[4],
                    med[5], 1, datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                ))
                total_meds += 1
    ...
    for elder in ELDERS:
        ...
        for _ in range(random.randint(2, 4)):
            try:
                ...
                cur.execute("""
                    INSERT INTO alerts (id, person_id, business_chain, alert_type, severity, status, message, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
